...
- 每个 task 开工前自动 grep `.flow-engine/sflow/lessons.md`，调试退出时自动追加经验教训。带标签索引和关键词匹配。

> 借鉴价值 ⭐⭐⭐⭐⭐：在 `.flow-engine/sflow/` 下增加 `lessons.md` 文件，每次 debugging 状态退出时自动写入"原因 + 方案 + 关键词"，`build-executor` 每个 task 开始前先扫。
...
sFlow 现状：状态主要在 `.flow-engine/sflow/` 下的文件推断，没有集中的跨会话状态文件。

> 借鉴价值 ⭐⭐⭐⭐：可在 opencode-sFlow 中增加 `.flow-engine/sflow/STATE.md` 或增强现有的 `.flow-engine/sflow/` 文件体系，增加明确的跨会话恢复入口。
...
需求澄清智能体：`packages/opencode-adapter/src/agents/need-explorer.ts` 提示词借鉴了 grilling 中的 3 点：
...
SFlow 当前的架构是 **Agent 驱动**的，是一个有自主决策能力的编排者；Spec-Kit 的 YAML 引擎是 **引擎驱动**的。把 YAML 步骤引擎搬过来会破坏 Agent 的自主决策能力。
...
spec-superflow 新增了 14 个提交实现完整的执行控制平面，这是最大、最值得借鉴的新功能。
...
spec-superflow 新增了 8 个提交实现 SDD overlay 和检查点系统。

| 组件 | 说明 | 借鉴价值 |
|------|------|----------|
| SDD Overlay 存储 | 持久化 overlay 目录，存储 SDD 执行状态 | ⭐⭐⭐⭐⭐ |
| 任务检查点 | `ssf checkpoint save|list|show` | ⭐⭐⭐⭐⭐ |
| 移交合约 | `ssf handoff create|list|show` | ⭐⭐⭐⭐⭐ |
...
- 在 `.flow-engine/sflow/` 下增加 overlay 存储目录结构
...
spec-superflow 新增了 8 个提交强化 review 收据的完整性验证。
...
**借鉴方向**：在 `code-reviewer` 完成 review 后，将 review 收据（commit hash 范围 + review 报告路径 + verdict）持久化到 `.flow-engine/sflow/reviews/` 目录，并验证收据完整性。

#### P4 — 最小化纪律（Minimality Discipline）

spec-superflow 新增了 4 个提交实现 `code-reviewer` 的最小化审查。
...
spec-superflow 新增了 6 个提交实现模型配置文件。
...
1. **执行控制平面** — 这是 spec-superflow 最大的架构改进，sFlow 的 `build-executor` 可以借鉴 wave 分波
2. **SDD 检查点升级** — sFlow 已有 `subagent-progress.md` 基础，升级到结构化检查点成本低
3. **Bug 验证** — closing gate 强制检查、幽灵状态检测、git 分支隔离，这些是质量保障的"最后一公里"
