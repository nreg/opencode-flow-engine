    ...
    async restoreState(changeDir: string): Promise<FeatureResult> {
      try {
        ...
        if (repairedState !== currentState) {
          Logger.log(`[SFLOW] Detected state mismatch: state=${currentState} but artifacts indicate ${repairedState}. Auto-repairing.`);
    ...
    async addLesson(changeDir: string, entry: import('./lessons.js').LessonEntry): Promise<FeatureResult> {
      try {
        ...
        if (entry.keywords.length < 3) {
          Logger.warn('[SFLOW] Lesson has fewer than 3 keywords. Consider adding more specific keywords for better searchability.');
        }
        ...
        if (looksAutoGenerated) {
          Logger.warn('[SFLOW] Lesson keywords appear auto-generated. Consider manually specifying more descriptive keywords (e.g., technology names, error codes, file paths).');
        }
