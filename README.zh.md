...
| **code-reviewer** | 子智能体 | 对照规格审查代码质量。强制实施**最小化审查纪律**（5 条必过门禁：禁止过度工程、禁止向后兼容垫片、禁止不必要抽象、禁止不必要配置、审查员安全阀） |
...
| **iflow-plan-executor** | 子智能体 | 执行器：4 条偏差规则（自动修复 bug → 自动补全关键功能 → 自动修复阻塞 → 架构变更需询问）。可委托前端任务给 `ui-implementer` |
...
| **ui-implementer** | 子智能体 | 前端 UI 实现，融合 9 项前端专业技能。与 sFlow 共享 |
...
| 技能来源 | 作用 | 说明 |
|---------|------|------|
| **[taste-skill](https://github.com/Leonxlnx/taste-skill)** | 设计品味控制 | 三旋钮设计系统、Design Read、AI 反模式禁令 |
| **[impeccable](https://github.com/pbakaus/impeccable)** | 审查修复 | 生产级设计准则、Absolute Bans、交互规范 |
...
- **iflow-plan-executor 委托** — 在 iFlow 执行模式中，前端任务自动路由到 ui-implementer
...
| `record_execution_plan` | 创建/更新执行计划（`.sflow/execution-plan.json`），含波次结构和模式选择 |
| `record_review_receipt` | 记录波次审查结果（`.sflow/reviews/<wave-id>.json`） |
...
`build-executor` 支持三种执行模式，在 DP-4（bridging→approved-for-build 转换）时根据任务数量和依赖分析自动推荐：
...
**条件**：任务 3-5 个且全部在同一个模块内，无 API/Schema 变更，无跨模块依赖
**行为**：一次完成整个批次，每步仍执行 TDD 红-绿-重构循环
**适用**：同一模块内的多项小改动

### 3. SDD 模式（子智能体驱动开发）

**条件**：6+ 个任务、跨模块变更、或有依赖链的任务
**行为**：
1. 为每个任务派发独立的 implementer 子智能体
2. 每完成一批次进行 spec 合规 + 代码质量审查（审查收据持久化到 `.sflow/reviews/`）
3. 最终进行全局审查
...
.sflow/checkpoints/          # 结构化检查点，含提交证据（saveCheckpoint, readCheckpoint, detectStaleCheckpoints）
.sflow/handoffs/             # 跨会话移交合约（createHandoff, finishHandoff, resolveHandoff）
```

### 执行控制平面

SDD 模式执行通过结构化的**执行计划**（`.sflow/execution-plan.json`）来编排：

- **波次调度**：任务按波次分组，支持 serial/parallel 策略和显式 depends_on 依赖
- **波次依赖校验**：Guard 钩子在执行前检测循环依赖和缺失波次引用
- **审查收据**：每个波次的审查结果持久化到 `.sflow/reviews/<wave-id>.json`，含提交范围证据
- **三重哈希校验**：执行计划通过内容哈希 + artifacts_hash + contract_hash 完整性验证
- **关闭门禁**：所有波次审查收据必须为 pass 状态才能进入 closing 状态
...
- **波次执行**：任务按波次分组，支持 serial/parallel 策略和依赖追踪
- **审查收据**：每个波次的审查结果持久化，含提交范围证据（`.sflow/reviews/`）
- **三重哈希校验**：内容哈希 + artifacts_hash + contract_hash 完整性检查
- **关闭门禁强制**：所有波次审查必须通过才能进入关闭状态

### 检查点与移交

- **检查点系统**：结构化任务检查点，含提交证据，通过合约哈希比较检测过期
- **移交合约**：跨会话移交，生命周期：创建 → 完成 → 解析（接受/拒绝/推迟）
- 检查点存储在 `.sflow/checkpoints/`，移交存储在 `.sflow/handoffs/`

### 模型 Profile

- 4 层模型解析：override → config → profile → fallback → default
- 4 个 profile：mechanical（快速）、standard（均衡）、strong（强力）、review（审查）
- 仅对 sFlow 工作流生效（iFlow 智能体跳过 profile 层）

### 技能压缩

- build-executor 和 code-reviewer 的 prompt 压缩约 40-50%，保留所有强制规则
- code-reviewer 最小化审查纪律：5 道防过度工程门禁
...
3. **`.sflow/config.json` agents 配置段**（按智能体配置模型）
4. **模型 Profile**（`.sflow/config.json` → `modelProfiles` 段 — 按角色映射到 profile 模型：mechanical、standard、strong、review）
5. **代码内建的 DEFAULT_MODELS**

模型不可用时，按备用模型列表依次尝试。

### 模型 Profile

`modelProfiles` 配置在 per-agent 配置和代码内建模型之间提供了一层兜底：

| Profile | 用途 | 典型智能体 |
|---------|------|-----------|
| `mechanical` | 快速廉价模型，用于简单任务 | release-archivist |
| `standard` | 均衡模型，用于通用工作 | sFlow, need-explorer, spec-merger, ui-implementer |
| `strong` | 强力模型，用于复杂推理 | spec-writer, contract-builder, build-executor, bug-investigator |
| `review` | 审查专用模型 | code-reviewer |

Profile 解析仅对 sFlow 工作流生效（iFlow 智能体跳过此层）。配置方式：
