...
**判断规则：**
1. 如果审查结果中没有任何 P0/P1 问题，且 P2/P3 问题数量很少（≤3 个且均为 nitpick）→ **结束循环**（P0/P1 必须修复；P2 视情况修复；P2/P3 数量少时仍可主动停止循环）
2. 如果存在 P0 或 P1 问题 → **进入 FIXING 状态**，修复所有 P0 + 你认为值得修的 P1（P0/P1 必须修复）
3. 如果存在 P2 问题但无 P0/P1，或用户可能有特定诉求 → **视情况修复**（数量少且改动风险低时可顺带修复），否则**咨询用户**决定是否继续；P3 直接跳过
...
- **提前停止条件：**
  ...
  - 审查结果全部为 P2/P3 且数量少 → 你（sFlow）主动停止（P2 视情况修复，P2/P3 数量少时仍可主动停止循环）
  - 用户主动要求停止
