...
> 详细内容见 `references/performance-accessibility.md`。包含：硬件加速规则、`prefers-reduced-motion` 强制要求、暗色模式策略、Core Web Vitals 目标、DOM 成本控制、Z-Index 约束。

## 7. DIAL DEFINITIONS (Technical Reference)

> 详细内容见 `references/dial-definitions.md`。包含：DESIGN_VARIANCE / MOTION_INTENSITY / VISUAL_DENSITY 三个旋钮的 1-10 级技术定义。

## 8. DARK MODE PROTOCOL

> 详细内容见 `references/dark-mode.md`。包含：Token 策略（Tailwind dark: vs CSS variables）、对比度要求、品牌保真度、默认模式选择。

## 9. AI TELLS (Forbidden Patterns)

> 详细内容见 `references/ai-tells.md`。包含 9 大类 100+ 条禁止模式：视觉/CSS、排版、布局、内容数据、外部资源、生产测试特征、装饰文本、列表评分、语言时间线索、**em-dash 完全禁令**。

## 10. REFERENCE VOCABULARY (Pattern Names the Agent Should Know)

> 详细内容见 `references/vocabulary.md`。包含 7 大类 50+ 个模式名：Hero 范式、导航菜单、布局网格、卡片容器、滚动动画、画廊媒体、排版效果、微交互库、动画库选择。
...
> 详细内容见 `references/block-library.md`。包含：block 文件位置 schema、Required Frontmatter 格式、Required Body 8 个必要段、Block-Library 纪律。

## 13. OUT OF SCOPE

> 详细内容见 `references/out-of-scope.md`。本 skill 不适用的场景：Dashboard/Admin、Data Table、多步骤表单、代码编辑器、原生移动端、实时协作 UI。

## 14. FINAL PRE-FLIGHT CHECK

> 详细内容见 `references/pre-flight.md`。包含 60+ 项检查清单：Brief 推断、Dial 值、Design System、Redesign 模式、em-dash 零容忍、页面主题锁、颜色一致性锁、形状一致性锁、CTA 对比度、Hero 适配、AI Tells 扫描、Motion 合理性、Dark Mode 验证、Mobile 折叠、Core Web Vitals 等。
