...
const handoff = formatStandardHandoff('normal', {
  ...
  completedWork: 'Root cause identified: null pointer in auth middleware',
  nextStage: 'executing',
  entryCondition: 'Fix applied and tests pass',
});
...
- Current stage: `debugging`.
- Completed / blocker: `Root cause identified: null pointer in auth middleware`.
- Next stage: `executing`.
- Entry condition: `Fix applied and tests pass`.
```
